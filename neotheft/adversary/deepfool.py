...
def deepfool_choose(target_model: Module, blackbox: Blackbox, queryset, testset, selection: set, transferset: List, indices_list: List, device, penalty: ndarray, **params):
    ...
    for index in tqdm(unselected):
        permutation, _, _, _, _ = deepfool(queryset[index][0], surrogate, penalty,ispenalty, num_classes)
        # batch_samples.append(result.squeeze(0))
        batch_permutation.append(np.linalg.norm(permutation))
    batch_permutation = np.array(batch_permutation)
...
def main():
    ...
    parser.add_argument('-w', '--num-workers', metavar='N', type=int, help='# Worker threads to load data',
                        default=10)
    parser.add_argument('-pen', '--ispenalty', metavar='N', type=bool, help='# use penalty matrix',
                        default=True)
    ...
    deepfool_choose(model, blackbox, queryset, testset, selection, transfer, indices_list, device, penalty, **params)
